...
| `release.yml` | tags `v*`                                                                                                                              | Creates a GitHub Release and uploads prebuilt binaries for macOS, Linux (glibc + fully static musl), Windows, iOS, and Android, each built with both the MSRV (1.97.1) and latest stable Rust. Archives are named `<bin>-<target>-rust-<msrv                                                                           | stable>` and ship with SHA-256 checksums, so users without Rust can download and run the binary directly |
